...
export class NewIncomesComponent {
  ...
  getListOfIncomesWithDate() {
    ...
    return this.incomeService
      ...
      .subscribe({
        next: (res) => {
          const filtered = res.incomes.filter((inc) => inc.checked === 1);

          this.incomesList = filtered;
  ...
  // MAKE THE INCOME CHECK STATUS SUCCESSFULL
  succCheck(incId: number) {
    this.selectedIncomeId = incId;
    Swal.fire({
      title: "Muvaffaqiyatli deb belgilash",
      text: `Rostdan ham ushbu Kirimni muvaffaqiyatli deb tasdiqlaysizmi??`,
      icon: "question",
      ...
      if (result.isConfirmed) {
        this.incomeService.checkStatus(incId, 2).subscribe({
          next: (res) => {
            Swal.fire({
              title: "Tasdiqlandi!",
          ...
          complete: () => {
            this.selectedIncomeId = null; // reset after action
          },
        });
      } else {
  ...
  // MAKING CHECK STATUS SUSPICIOUS
  susCheck(incId: number) {
    this.selectedIncomeId = incId;
    Swal.fire({
      title: "Shubhali deb belgilash",
      text: `Rostdan ham ushbu Kirimni SHUBHALI deb tasdiqlaysizmi??`,
      icon: "question",
      ...
      if (result.isConfirmed) {
        this.incomeService.checkStatus(incId, 3).subscribe({
          next: (res) => {
            Swal.fire({
              title: "Tasdiqlandi!",
  ...
  // MAKE CHECK STATUS FAILED

  failedCheck(incId: number) {
    this.selectedIncomeId = incId;
    Swal.fire({
      title: "Shubhali deb belgilash",
      text: `Rostdan ham ushbu Kirimni Muvaffaqiyatsiz deb tasdiqlaysizmi??`,
      icon: "question",
      confirmButtonText: "Ha, Saqlash",
      showCancelButton: true,
      cancelButtonText: "Yo'q, Bekor Qilish",
      ...
      if (result.isConfirmed) {
        this.incomeService.checkStatus(incId, 4).subscribe({
          next: (res) => {
            Swal.fire({
              title: "Tasdiqlandi!",
