...
export class FailedIncomesComponent implements OnInit {
  incomesList: any[] = [];
  incomeCats: any[] = [];
  errorMessage: string = "";
  ...
  loadIncomes(): void {
    ...
    this.incomeService.getIncomes(this.currentPage).subscribe({
      next: (res) => {
        // Filter only failed incomes (checked = 4)
  ...
  downloadExcel(): void {
    ...
    this.exportToExcel(exportData, "Muvaffaqiyatsiz-Kirimlar");
  }
