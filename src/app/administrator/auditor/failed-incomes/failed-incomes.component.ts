...
@Component({
  selector: "app-failed-incomes",
  templateUrl: "./failed-incomes.component.html",
  styleUrls: ["./failed-incomes.component.css"],
})
export class FailedIncomesComponent {
  ...
  loadIncomes() {
    this.incomeService.getIncomes(this.currentPage).subscribe({
      next: (res) => {
        this.incomesList = res.incomes.filter((inc) => inc.checked === 4);
  ...
  getListOfIncomesWithDate() {
    ...
    return this.incomeService
      ...
      .subscribe({
        next: (res) => {
          const filtered = res.incomes.filter((inc) => inc.checked === 4);
  ...
  downlaodExcel() {
    if (this.filteredIncomes.length) {
      ...
      this.exportToExel(filteredData, "Muvaffaqiyatsiz-Kirimlar");
      return;
    }
    alert("No data");
  }
