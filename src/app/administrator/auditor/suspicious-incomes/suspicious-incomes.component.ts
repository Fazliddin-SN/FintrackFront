...
export class SuspiciousIncomesComponent {
  ...
  getListOfIncomesWithDate() {
    ...
    return this.incomeService
      ...
      .subscribe({
        next: (res) => {
          const filtered = res.incomes.filter((inc) => inc.checked === 3);
  ...
  downlaodExcel() {
    if (this.filteredIncomes.length) {
      ...
      return;
    } else if (this.incomesList.length) {
      const filteredData = this.incomesList.map((exp) => ({
        Somda: this.formatCurrency(+exp.uzs_cash) || 0,
        Dollarda: this.formatCurrency(+exp.usd_cash) || 0,
        Kartaga: this.formatCurrency(+exp.card) || 0,
        "Kompaniya Hisobiga": this.formatCurrency(+exp.account) || 0,
      ...
      this.exportToExel(filteredData, "Shubhali-Kirimlar");
      return;
    } else {
      alert("No data");
    }
